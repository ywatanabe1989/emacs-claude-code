...
(defun ecc-claude-help ()
  ...
  (with-help-window "*Claude Help*"
    ...
    ;; Basic operations
    (princ "C-c c v      ecc-claude-vterm        Start a new Claude vterm session\n")
    (princ "C-c c o      ecc-claude-optimize     Optimize current vterm for Claude\n")
    
    ;; Auto-response
    (princ "C-c c a      ecc-toggle-auto         Toggle auto-response on/off\n")
    (princ "C-c c q      ecc-quick-auto          Quick enable auto-response with /auto\n")
    (princ "C-c c s      ecc-claude-stop-auto    Emergency stop auto-response\n")
    ...
    ;; Notifications
    (princ "C-c c n      ecc-notify-toggle       Toggle prompt notifications\n")
    (princ "C-c c b      ecc-bell-toggle         Toggle notification bell\n")
    
    ;; Interaction tracking and periodic commands
    (princ "C-c c i      ecc-interaction-stats   Show interaction statistics\n")
    (princ "C-c c p      ecc-toggle-periodic     Toggle periodic commands\n")
    (princ "C-c c c      ecc-add-periodic-cmd    Add a new periodic command\n")
    ...
    (princ "==================\n\n")
    (princ "The auto-response system automatically responds to Claude prompts:\n")
    ...
    (princ "==================\n\n")
    (princ "The notification system alerts you when Claude prompts are detected:\n")
    ...
    (princ "- Default and step sizes configurable through ecc-vterm-* variables\n\n")
    
    ;; Yank-as-file Feature
    (princ "Yank-as-File Feature\n")
    (princ "===================\n\n")
    (princ "Save Claude's output to files with automatic file type detection:\n")
    (princ "- Yank region to file with C-c C-y (prompts for filename)\n")
    (princ "- Yank entire buffer to file with C-c C-w\n")
    (princ "- Quick yank region to auto-named file with C-c C-q\n")
    (princ "- Auto-detects appropriate file extension based on content\n")
    (princ "- Supports many languages including Python, JavaScript, HTML, CSS, etc.\n")
    (princ "- Customizable default directory and extensions through ecc-vterm-yank-* variables\n\n")
    
    ;; Eye-Friendly Features
    (princ "Eye-Friendly Features\n")
    (princ "====================\n\n")
    (princ "Reduce eye strain with these features (C-c c e to toggle):\n")
    (princ "- Smooth scrolling: Buffer moves smoothly instead of jumping\n")
    (princ "- Scroll speed control: Limit maximum scrolling speed (C-c c r)\n")
    (princ "- Visual indicator: Shows scroll speed with color feedback\n")
    ...
    (princ "- Supports periodic commands based on interaction count\n\n")
    
    ;; Periodic Commands System
    (princ "Periodic Commands System\n")
    (princ "======================\n\n")
    (princ "Automatically sends commands at specified intervals:\n")
    (princ "- Default: /compact every 10 interactions, /git every 20\n")
    ...
    (princ "Smart scrolling is enabled - your position will be preserved when reading\n")
    (princ "earlier content in the buffer.\n\n")
    (princ "If you need to stop auto-response immediately, use C-c c s.\n")))
...
When enabled, provides keybindings for Claude interaction commands."
  ...
  :keymap (let ((map (make-sparse-keymap)))
            ...
            ;; Auto-response
            (define-key map (kbd "C-c c a") 'ecc-toggle-auto-response)
            (define-key map (kbd "C-c c q") 'ecc-claude-quick-auto-response)
            ...
            (define-key map (kbd "C-c c i") 'ecc-claude-interaction-stats)
