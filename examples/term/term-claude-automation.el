...
;;; Author: ywatanabe
;;; Timestamp: <2025-05-13 05:41:00>
;;; File: /home/ywatanabe/.emacs.d/lisp/emacs-claude-code/examples/term/term-claude-automation.el
...
(defun example-start-claude-term-session ()
  ...
  (let ((buffer (ecc-term-run-claude)))
    (with-current-buffer buffer
      ...
      (message
       "Claude terminal session started with auto-mode enabled"))))
...
(defun example-setup-automated-claude-responses ()
  ...
  (unless (eq major-mode 'ecc-term-claude-mode)
    (error
     "This function must be run in an ecc-term-claude-mode buffer"))
  ...
  (setq-local ecc-term-claude-prompt-initial-waiting
  ...
  (add-to-list 'ecc-term-claude-update-functions
               'my-custom-claude-auto-responder)
...
(defun example-send-file-to-claude-term (file-path)
  ...
  (let
      ((progress-buffer (get-buffer-create "*Claude Send Progress*")))
    ...
    (let* ((file-content (with-temp-buffer
      ...
      (let ((claude-buffer (car (seq-filter
        ...
        (let* ((chunk-size (ecc-buffer-large-get-optimal-chunk-size))
               (chunks
                (ecc-buffer-large-chunk-string file-content chunk-size)))
            ...
            (dolist (chunk chunks)
              ...
              (with-current-buffer progress-buffer
                (let ((inhibit-read-only t))
                  (goto-char (point-max))
                  (insert
                   (format "Sending chunk %d/%d... " i (length chunks)))))
              ...
              (with-current-buffer claude-buffer
                (if (= i 1)
                    ...
                    (let
                        ((prompt
                          (read-string
                           "Enter prompt for Claude (optional): ")))
                      (ecc-term-send-string (if
  ...
  (let ((buffer (get-buffer-create "*Claude Terminal Dashboard*")))
    (with-current-buffer buffer
      ...
      (insert-button "[Start New Session]"
                     ...
                     'help-echo
                     "Start a new Claude terminal session with auto-mode")
      (insert "  ")
      ...
      (insert-button "[Configure Auto-Responses]"
                     'action (lambda (_)
                               (let
                                   ((buf (get-buffer "*CLAUDE-VTERM*")))
                                 (if buf
                                     (progn
                                       (switch-to-buffer buf)
                                       (example-setup-automated-claude-responses))
                                   (message
                                    "No Claude terminal buffer found"))))
                     'help-echo
                     "Configure automated responses in current Claude terminal")
      (insert "\n\n")
      ...
      (insert-button "[Send File to Claude]"
                     'action (lambda (_)
                               (call-interactively
                                #'example-send-file-to-claude-term))
                     'help-echo
                     "Send a file to Claude with progress tracking")
      (insert "  ")
                     ...
                     'action (lambda (_)
                               (let ((content (save-excursion
                                 ...
                                 (if (fboundp 'ecc-term-send-string)
                                   ...
                                   (message
                                    "ecc-term-send-string not available"))))
                     'help-echo
                     "Send content from other buffer to Claude")
      (insert "\n\n")
      ...
      (insert
       "The `ecc-term-claude-mode` (previously `ecc-claude-vterm-mode`) provides:\n\n")
  ...
  (if example-global-claude-term-mode
      (progn
        ...
        (add-hook 'vterm-mode-hook
                  #'example-detect-and-setup-claude-term)
...
(when
    (not load-file-name)
  (message "term-claude-automation.el loaded."
