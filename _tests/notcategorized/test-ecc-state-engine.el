...
;;; Author: ywatanabe
;;; Timestamp: <2025-05-13 02:08:56>
;;; File: /home/ywatanabe/.emacs.d/lisp/emacs-claude-code/tests/notcategorized/test-ecc-state-engine.el
...
(ert-deftest test-ecc-state-engine-detection ()
  ...
  (let
      ((running-pattern (ecc-state-prompt-pattern ecc-state-running)))
  ...
  (let
      ((waiting-pattern (ecc-state-prompt-pattern ecc-state-waiting)))
...
(ert-deftest test-ecc-state-engine-hooks ()
  ...
  (let ((enter-hook-called nil)
    ...
    ;; Add hooks
    (ecc-state-engine-add-hook 'idle 'exit
                               (lambda () (setq exit-hook-called t)))
    (ecc-state-engine-add-hook 'running 'enter
                               (lambda () (setq enter-hook-called t)))
    ...
    ;; Remove hooks
    (ecc-state-engine-remove-hook 'idle 'exit
                                  (lambda () (setq exit-hook-called t)))
    (ecc-state-engine-remove-hook 'running 'enter
                                  (lambda ()
                                    (setq enter-hook-called t)))
...
(when
    (not load-file-name)
  (message "test-ecc-state-engine.el loaded."
