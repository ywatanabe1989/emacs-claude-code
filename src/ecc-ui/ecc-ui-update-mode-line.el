...
;;; Author: ywatanabe
;;; Timestamp: <2025-05-13 18:18:33>
;;; File: /home/ywatanabe/.emacs.d/lisp/emacs-claude-code/src/ecc-ui/ecc-ui-update-mode-line.el
...
(defvar ecc-mode-line-indicator " [Auto Mode]"
...
(defun ecc-ui-update-mode-line (enable)
    ...
    (while tail
      (let ((buf (car tail)))
        (when (buffer-live-p buf)
          (with-current-buffer buf
            (let*
                ((is-active (eq buf ecc-buffer-current-active-buffer))
                 (state-color (cond
                               (ecc-state-running-p "green")
                               (is-active "orange")
                               (t "gray"))))
...
(provide 'ecc-ui-update-mode-line)

(when
    (not load-file-name)
  (message "ecc-ui-update-mode-line.el loaded."
