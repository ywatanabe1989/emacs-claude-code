...
;;; File: /home/ywatanabe/.emacs.d/lisp/emacs-claude-code/src/ecc-ui/ecc-ui-history.el
...
(define-derived-mode ecc-history-mode special-mode
  "ecc-history"
...
Returns the browser buffer."
  (let*
      ((buffer-name
        (format "*Claude History: %s*" (buffer-name buffer)))
    ...
    (with-current-buffer browser-buffer
      ...
      ;; Header
      (insert
       (format "History for buffer: %s\n\n" (buffer-name buffer)))
      (insert
       "Press RET to send a prompt, r to refresh, q to quit\n\n")
  ...
  (let ((source-buffer ecc-history-source-buffer))
    (if (buffer-live-p source-buffer)
        (let ((browser-buffer (current-buffer))
          ...
          (let
              ((new-buffer (ecc-history-browser-create source-buffer)))
...
(when (fboundp '--ecc-send-string)
  (advice-add '--ecc-send-string :around
              #'--ecc-history-after-send-advice))


(provide 'ecc-ui-history)

(when
    (not load-file-name)
  (message "ecc-ui-history.el loaded."
