...
;;; Author: ywatanabe
;;; Timestamp: <2025-05-20 08:04:49>
;;; File: /home/ywatanabe/.emacs.d/lisp/emacs-claude-code/src/ecc-vterm/ecc-vterm-mode.el
...
for handling high-throughput interactions with Claude."
  ...
  :keymap (let ((map (make-sparse-keymap)))
            ...
            (define-key map (kbd "C-c C-o") 'ecc-vterm-toggle-optimizations)
            
            ;; Visual settings
            (define-key map (kbd "C-c C-l") 'ecc-vterm-toggle-line-numbers)
            
            ;; Font size adjustment
            (define-key map (kbd "C-c C-+") 'ecc-vterm-increase-font-size)
            (define-key map (kbd "C-c C-=") 'ecc-vterm-increase-font-size) ; Alternative binding
            (define-key map (kbd "C-c C--") 'ecc-vterm-decrease-font-size)
            (define-key map (kbd "C-c C-0") 'ecc-vterm-reset-font-size)
            
            ;; Mouse wheel bindings for font size
            (define-key map (vector (list 'control 'mouse-4)) 'ecc-vterm-increase-font-size)
            (define-key map (vector (list 'control 'mouse-5)) 'ecc-vterm-decrease-font-size)
...
(defun ecc-vterm--disable ()
  ...
  (when ecc-vterm--saved-config
    ...
    (font-lock-mode (if (plist-get ecc-vterm--saved-config :font-lock-mode) 1 -1))
    ...
    (let ((saved-scale (plist-get ecc-vterm--saved-config :text-scale)))
      (when saved-scale
