...
;;; Author: ywatanabe
;;; Timestamp: <2025-06-04 08:47:03>
;;; File: /home/ywatanabe/.emacs.d/lisp/emacs-claude-code/src/ecc-auto-response-logging.el
...
Example:
  ...
  (when (>= (cdr (assq level --ecc-auto-response-log-levels))
            (cdr
             (assq ecc-auto-response-log-level
                   --ecc-auto-response-log-levels)))
    (let* ((timestamp (format-time-string "%Y-%m-%d %H:%M:%S.%3N"))
      ...
      ;; Trim if too many
      (when
          (> (length --ecc-auto-response-log-entries)
             ecc-auto-response-log-max-entries)
        (setq --ecc-auto-response-log-entries
              (cl-subseq --ecc-auto-response-log-entries 0
                         ecc-auto-response-log-max-entries)))
      ;; Also log to debug if enabled
      (when --ecc-debug-enabled
        (--ecc-debug-message "[%s] %s: %s" level buffer-name
                             formatted-message))
...
(defun ecc-auto-response-show-log ()
  ...
  (interactive)
  (let
      ((buffer (get-buffer-create ecc-auto-response-log-buffer-name)))
...
(when (not load-file-name)
  (--ecc-debug-message "ecc-auto-response-logging.el loaded."
