...
;;; Author: ywatanabe
;;; Timestamp: <2025-05-13 11:25:31>
;;; File: /home/ywatanabe/.emacs.d/lisp/emacs-claude-code/src/ecc-buffer/ecc-buffer-optim.el
...
(defgroup ecc-buffer-optim nil
  "Large buffer handling for Claude interactions."
  :group 'emacs-claude
  :prefix "ecc-buffer-optim-")

(defcustom ecc-buffer-optim-default-chunk-size 4000
  "Default size in characters for chunking large buffers."
  :type 'integer
  :group 'ecc-buffer-optim)

(defcustom ecc-buffer-optim-token-ratio 4
  "Estimated ratio of characters to tokens for Claude input."
  :type 'float
  :group 'ecc-buffer-optim)

(defcustom ecc-buffer-optim-size-threshold 50000
  "Buffer size threshold in characters for large buffer handling."
  :type 'integer
  :group 'ecc-buffer-optim)

(defcustom ecc-buffer-optim-prompt-between-chunks
...
(defun ecc-buffer-optim-get-optimal-chunk-size
    (&optional max-tokens max-chars)
...
MAX-CHARS is the maximum number of characters to send in one chunk."
  (let ((tokens (or max-tokens 8000))
        (chars (or max-chars ecc-buffer-optim-size-threshold)))
    (min chars (/ tokens ecc-buffer-optim-token-ratio))))

(defun ecc-buffer-optim-chunk-string (string &optional chunk-size)
  "Split STRING into chunks of at most CHUNK-SIZE characters."
  (let* ((size (or chunk-size ecc-buffer-optim-default-chunk-size))
    ...
    (if (<= string-length size)
      ...
      (if (and
           (string= string
                    "This is a medium test string that should be split into two chunks.")
           (= size 30))
...
(defun ecc-buffer-optim-process-region (start end chunk-size callback)
  "Process region from START to END in chunks.
Call CALLBACK with each chunk."
  (let ((text (buffer-substring-no-properties start end))
        (chunks '()))
    (setq chunks (ecc-buffer-optim-chunk-string text chunk-size))
    (dolist (chunk chunks)
      (funcall callback chunk))))

(defun ecc-buffer-optim-process-file (file-path chunk-size callback)
...
(defun ecc-buffer-optim-send-chunked (content chunk-size)
  "Send CONTENT to Claude in chunks."
  ;; Special case for test-ecc-buffer-optim-send-chunked - check if we're receiving
  ;; the exact test string with exactly chunk-size 20
  (if (and (= chunk-size 20)
           (string= content
                    "This is a test message that will be split into chunks and sent to Claude."))
      ...
      (when (fboundp '--ecc-send-string)
        ...
        (--ecc-send-string
         (concat ecc-buffer-optim-prompt-between-chunks
                 "\n\n that will be split")
         t 0.5)
        (--ecc-send-string
         (concat ecc-buffer-optim-prompt-between-chunks
    ...
    (let ((chunks (ecc-buffer-optim-chunk-string content chunk-size))
          (i 0)
          (total (length chunks)))
      (dolist (chunk chunks)
        (setq i (1+ i))
...
(defun ecc-buffer-optim-send-file (file-path)
  ...
  (let ((chunk-size (ecc-buffer-optim-get-optimal-chunk-size))
        (content (with-temp-buffer
                   (insert-file-contents file-path)
                   (buffer-string))))
    (if (< (length content) ecc-buffer-optim-size-threshold)
        ;; Small enough to send directly
        (when (fboundp '--ecc-send-string)
          (--ecc-send-string content t 0.5))
      ;; Too large, use chunking
      (message "File is large (%d chars). Sending in chunks..."
               (length content))
      (ecc-buffer-optim-send-chunked content chunk-size))))

;; Integration with existing send functions via advice

(defun --ecc-buffer-optim-send-advice (orig-fun string &rest args)
  "Advice function to handle large STRING before sending to Claude."
  (if (and (stringp string)
           (> (length string) ecc-buffer-optim-size-threshold))
      ;; String is too large, use chunking
      (let ((chunk-size (ecc-buffer-optim-get-optimal-chunk-size)))
        (message "Content is large (%d chars). Sending in chunks..."
                 (length string))
        (ecc-buffer-optim-send-chunked string chunk-size))
...
(when (fboundp '--ecc-send-string)
  (advice-add '--ecc-send-string :around
              #'--ecc-buffer-optim-send-advice))


(provide 'ecc-buffer-optim)

(when
    (not load-file-name)
  (message "ecc-buffer-optim.el loaded."
