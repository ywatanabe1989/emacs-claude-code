...
(defcustom ecc-vterm-yank-extension-alist
  '(("py" . "^\\(import\\|from\\|def\\|class\\|if __name__ ==\\)")
    ...
    ("java"
     . "\\(public\\s-+\\(class\\|interface\\)\\|import\\s-+java\\)")
    ("c" . "\\(#include\\|int\\s-+main\\)")
    ("cpp"
     . "\\(#include\\|namespace\\|template\\|class\\s-+[a-zA-Z_][a-zA-Z0-9_]*\\)")
    ("rb" . "\\(require\\|def\\|class\\|module\\|gem\\)")
    ("php" . "\\(<\\?php\\|function\\s-+[a-zA-Z0-9_]+\\s-*(\\)")
    ("ts"
     . "\\(interface\\|type\\|class\\|enum\\|namespace\\|import\\)")
...
Auto-detects the file type based on content if no extension is provided."
  (interactive
   (if (region-active-p)
       (let* ((content (buffer-substring-no-properties
              ...
              (filename (read-file-name "Save to file: " dir nil nil
                                        default-name
  ...
  (let
      ((file-path
        (ecc-vterm-generate-file-from-region start end filename)))
...
Auto-detects the file type based on content if no extension is provided."
  (interactive
   (let* ((content (buffer-substring-no-properties
          ...
          (filename (read-file-name "Save buffer to file: " dir nil
                                    nil default-name
                                    'ecc-vterm-yank-history)))
     (list filename)))
