...
;;; Author: ywatanabe
;;; Timestamp: <2025-05-14 05:57:22>
;;; File: /home/ywatanabe/.emacs.d/lisp/emacs-claude-code/src/ecc-term/ecc-term-mode.el

;;; Copyright (C) 2025 Yusuke Watanabe (ywatanabe@alumni.u-tokyo.ac.jp)


;; Error running timer ‘ecc-vterm--detect-state’: (void-function ecc-state-engine-detect-state) [25 times]
...
(require 'ecc-term-follow-bottom)
...
Key bindings:
\\{ecc-term-vterm-mode-map}"))
;; Common setup functions for both variants of the mode
(defun ecc-term-vterm-mode-common-setup ()
  "Common setup for ecc-term-vterm-mode regardless of parent mode."
  ...
  (setq-local scroll-conservatively
              ecc-term-vterm-scroll-conservatively
              scroll-margin 0
              scroll-step 1
  ...
  ;; Initialize follow-bottom feature
  (when ecc-term-vterm-always-follow-bottom
    (ecc-term-follow-bottom-setup)))

;; Add common setup to both mode variants
(add-hook 'ecc-term-vterm-mode-hook 'ecc-term-vterm-mode-common-setup)
  ...
  (let ((menu (make-sparse-keymap "Claude-VTerm")))
    ...
    (let ((submenu (make-sparse-keymap "Buffer Navigation")))
      ...
      (define-key submenu [ecc-term-vterm-prev-buffer]
                  '(menu-item "Previous Buffer"
                              ecc-term-vterm-prev-buffer
                              :help
                              "Switch to the previous Claude buffer"))
      (define-key menu [ecc-buffer-navigation]
                  (cons "Buffer Navigation" submenu)))

    ;; Display options submenu
    (let ((submenu (make-sparse-keymap "Display Options")))
      (define-key submenu [ecc-term-vterm-follow-bottom-toggle]
      ...
      (define-key submenu [ecc-term-vterm-visual-aid-toggle]
                  '(menu-item "Visual Aids" 
                              ecc-term-visual-aid-toggle
                              :help "Toggle visual highlighting for Claude state"
                              :button (:toggle . ecc-term-visual-aid-enabled)))
      (define-key submenu [ecc-term-vterm-visual-aid-customize]
                  '(menu-item "Customize Visual Aids..." 
                              ecc-term-visual-aid-customize
                              :help "Customize visual aid settings"))
      (define-key menu [ecc-term-display-options]
                  (cons "Display Options" submenu)))
...
(defvar ecc-term-vterm-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    ;; Add menu
    (define-key map [menu-bar claude-vterm]
                (cons "Claude" ecc-term-vterm-menu))
...
(defun ecc-term-vterm ()
  ...
  (unless ecc-term--vterm-available
    (error
     "Cannot create Claude vterm buffer: vterm package not available"))
...
This function is meant to be called when initializing VTERM mode."
  (when (boundp 'ecc-state-prompt-waiting)
    (setq-default ecc-state-prompt-waiting
                  ecc-term-vterm-prompt-waiting))
  ...
  (when (boundp 'ecc-state-prompt-initial-waiting)
    (setq-default ecc-state-prompt-initial-waiting
                  ecc-term-vterm-prompt-initial-waiting))

  ;; Also update regex patterns
  (when (boundp 'ecc-state-prompt-pattern-waiting)
    (setq-default ecc-state-prompt-pattern-waiting
                  ecc-term-vterm-prompt-waiting))

  (when (boundp 'ecc-state-prompt-pattern-y/n)
    (setq-default ecc-state-prompt-pattern-y/n
                  ecc-term-vterm-prompt-y/n))

  (when (boundp 'ecc-state-prompt-pattern-y/y/n)
    (setq-default ecc-state-prompt-pattern-y/y/n
                  ecc-term-vterm-prompt-y/y/n))

  (when (boundp 'ecc-state-prompt-pattern-initial-waiting)
    (setq-default ecc-state-prompt-pattern-initial-waiting
                  ecc-term-vterm-prompt-initial-waiting)))
...
(provide 'ecc-term-mode)

(when
    (not load-file-name)
  (message "ecc-term-mode.el loaded."
