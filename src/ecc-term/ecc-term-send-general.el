...
;;; Author: ywatanabe
;;; Timestamp: <2025-05-13 08:21:41>
;;; File: /home/ywatanabe/.emacs.d/lisp/emacs-claude-code/src/ecc-term/ecc-term-send-general.el
...
;;;###autoload
(defun ecc-term-send-general-interrupt ()
...
(defun ecc-term-send-general-paste-to-active-buffer ()
...
(defun ecc-term-send-general-template ()
...
;; (defun ecc-term-send-general-accept ()
...
;;   (interactive)
;;   (let ((response "1"))
;;     (--ecc-term-send-general-by-state response :y/n)
...
;;   (interactive)
;;   (let ((response "2"))
;;     (--ecc-term-send-general-by-state response :y/y/n)
;;     (when (fboundp 'ecc-auto-notify-completion)
...
;; Returns the string that was sent."
;;   (interactive)
;;   (--ecc-term-send-general-by-state custom-text :y/y/n)
;;   (when (fboundp 'ecc-auto-notify-completion)
...
;;     (--ecc-term-send-general-by-state
;;      response
;;      (lambda ()
...
(defun --ecc-term-send-general-string (string &optional no-confirm delay buffer)
...
Returns the string that was sent."
  (when (--ecc-state-is-claude-active-p buffer)
    (with-current-buffer ecc-buffer-current-active-buffer
      ...
      (vterm-copy-mode -1)
      (when delay (sit-for delay))
      (message "[ecc-term-send-general] Sent: %s" string)
      string)))  ;; Return the string that was sent

(defun --ecc-term-send-general-by-state (response state-or-predicate)
...
Returns the response string if sent, nil otherwise."
  (with-current-buffer ecc-buffer-current-active-buffer
    (let ((should-send
      ...
      (when should-send
        (--ecc-term-send-general-string response t 1.0)
        (vterm-send-return)
        (message "[ecc-term-send-general] Automatic Response: %s" response)
...
(defalias 'ecc-send-interrupt 'ecc-term-send-general-interrupt)
(defalias 'ecc-send-clear-history 'ecc-term-send-general-clear-history)
(defalias 'ecc-send-compact-history 'ecc-term-send-general-compact-history)
(defalias 'ecc-send-edit-memory 'ecc-term-send-general-edit-memory)
(defalias 'ecc-send-region 'ecc-term-send-general-region)
(defalias 'ecc-send-paste-to-active-buffer 'ecc-term-send-general-paste-to-active-buffer)
(defalias 'ecc-send-template 'ecc-term-send-general-template)
(defalias 'ecc-send-accept 'ecc-term-send-general-accept)
...
(defalias '--ecc-send-string '--ecc-term-send-general-string)
(defalias '--ecc-send-by-state '--ecc-term-send-general-by-state)
...
(provide 'ecc-term-send-general)

(when
    (not load-file-name)
  (message "ecc-term-send-general.el loaded."
