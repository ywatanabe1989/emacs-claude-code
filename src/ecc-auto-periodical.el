...
(defun ecc-auto-periodical-increment ()
  ...
  (when (and ecc-auto-periodical-enabled
             (or --ecc-auto-periodical-enabled-local
                 (not (local-variable-p '--ecc-auto-periodical-enabled-local))))
    (setq-local --ecc-auto-periodical-interaction-counter
                (1+ --ecc-auto-periodical-interaction-counter))
    (--ecc-debug-message "Interaction counter: %d" --ecc-auto-periodical-interaction-counter)
...
(defun ecc-auto-periodical-execute (command interval)
  ...
  (when (or (not ecc-auto-periodical-prompt-before-execute)
            (y-or-n-p (format "Execute periodic command '%s' (interval: %d)? " 
                              command interval)))
    ;; Record execution
    ...
    (when ecc-auto-periodical-notify
      (message "Executing periodic command: %s (every %d interactions)" 
               command interval))
    
    ;; Execute the command
    (ecc-auto-periodical--send-command command)
    
    (--ecc-debug-message "Executed periodic command: %s at interaction %d"
                         command --ecc-auto-periodical-interaction-counter)))
...
(defun ecc-auto-periodical-toggle-buffer ()
  ...
  (setq-local --ecc-auto-periodical-enabled-local
              (not (if (local-variable-p '--ecc-auto-periodical-enabled-local)
                       --ecc-auto-periodical-enabled-local
                     ecc-auto-periodical-enabled)))
  (message "Auto periodic commands %s in buffer %s"
           (if --ecc-auto-periodical-enabled-local "enabled" "disabled")
           (buffer-name)))
...
(defun ecc-auto-periodical-status ()
  ...
  (interactive)
  (let ((status-lines
         (list (format "Global: %s" (if ecc-auto-periodical-enabled "enabled" "disabled"))
               (format "Buffer: %s" (if (and (local-variable-p '--ecc-auto-periodical-enabled-local)
                                             --ecc-auto-periodical-enabled-local)
                                        "enabled" "disabled"))
               (format "Counter: %d" --ecc-auto-periodical-interaction-counter)
               "Commands:")))
    (dolist (entry ecc-auto-periodical-commands)
      (let* ((interval (car entry))
             (command (cdr entry))
             (last-exec (gethash command --ecc-auto-periodical-last-executed 0))
             (next-exec (* interval (1+ (/ --ecc-auto-periodical-interaction-counter interval)))))
