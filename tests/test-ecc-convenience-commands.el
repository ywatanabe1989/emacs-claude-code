              ...
              ((symbol-function 'fboundp)
               (lambda (sym)
                 (cond ((eq sym 'ecc-term-claude) nil)
                       ((eq sym 'vterm) t)
                       (t (funcall (symbol-function 'fboundp) sym))))))
...
(ert-deftest test-ecc-quick-auto-response-success ()
  ...
  (let ((auto-response-args nil))
    (cl-letf (((symbol-function 'ecc-auto-response-start)
              ...
              ((symbol-function 'fboundp)
               (lambda (sym)
                 (if (eq sym 'ecc-auto-response-start) t
                   (funcall (symbol-function 'fboundp) sym))))
              ((symbol-function 'message) #'test-ecc-mock-message))
      
      (ecc-quick-auto-response)
      ;; Should be called with specific auto-response arguments
      (should (equal auto-response-args '("y" "y" "/user:auto")))
      (should (member "Quick auto-response enabled (y/y, /user:auto)" test-ecc-messages)))))

;; Tests for ecc-status

(ert-deftest test-ecc-status-all-active ()
  "Test ecc-status when all systems are active."
  (test-ecc-reset-messages)
  (let ((ecc-auto-response-enabled t)
        (ecc-mode t))
    (cl-letf (((symbol-function 'message) #'test-ecc-mock-message)
              ((symbol-function 'boundp)
               (lambda (sym)
                 (cond ((eq sym 'ecc-auto-response-enabled) t)
                       ((eq sym 'ecc-mode) t)
                       ((eq sym 'ecc-buffer-auto-response-enabled) nil)
                       (t (funcall (symbol-function 'boundp) sym))))))
      
      (ecc-status)
      ;; Check that status message was displayed
      (should (cl-some (lambda (msg) (string-match-p "Mode: active" msg)) 
                       test-ecc-messages))
      (should (cl-some (lambda (msg) (string-match-p "Auto-response: active" msg)) 
                       test-ecc-messages)))))
