...
;;; Author: ywatanabe
;;; Timestamp: <2025-05-13 01:42:12>
;;; File: /home/ywatanabe/.emacs.d/lisp/emacs-claude-code/tests/buffer/test-ecc-buffer-state.el
        ...
        (progn
          ...
          (with-current-buffer test-buffer
            ...
            (cl-letf (((symbol-function 'derived-mode-p)
              ...
              ;; Test prompt detection
              (should
               (--ecc-state-detect-prompt ecc-prompt-pattern-waiting))
              (should
               (--ecc-state-detect-prompt ecc-prompt-pattern-y/n))
              (should-not
               (--ecc-state-detect-prompt ecc-prompt-pattern-y/y/n))
          ...
          (cl-letf (((symbol-function 'derived-mode-p)
                    ...
                    ((symbol-function '--ecc-state-detect-prompt)
                     (lambda (pattern buf &optional n-lines)
                       (when (and pattern (string= pattern "[Y/y/n]"))
                         t)))
                    ...
                    ((symbol-function '--ecc-state-is-waiting-p)
                     (lambda (buf) nil))
                    ((symbol-function
                      '--ecc-state-is-initial-waiting-p)
                     (lambda (buf) nil))
            ...
            (cl-letf (((symbol-function '--ecc-state-detect-prompt)
                       (lambda (pattern buf &optional n-lines)
                         (cond
                          ((and pattern (string= pattern "[Y/y/n]"))
                           nil)
                          ((and pattern
                                (string= pattern
                                         ecc-prompt-pattern-y/n))
                           t)
                          (t nil)))))
            ...
            (cl-letf (((symbol-function '--ecc-state-detect-prompt)
                       (lambda (pattern buf &optional n-lines)
                         (cond
                          ...
                          ((and pattern
                                (string= pattern
                                         ecc-prompt-pattern-initial-waiting))
                           t)
                          (t nil)))))
              (should
               (eq :initial-waiting (ecc-state-get test-buffer))))
